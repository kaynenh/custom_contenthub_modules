...
//    $entities = $entities_tracking->getEntitiesToReindex($entity_type_id);
    $entities = ibm_ach_ap_get_entities_tracking($entity_type_id, $status);
    break;
...
foreach ($chunks as $operation_key =>$chunk) {
  $operations[] = [
    'ibm_acquia_contenthub_audit_publisher',
...
if ($list = $client_manager->createRequest('listEntities', [$options])) {
  ...
  for ($i = 0; $i <= $steps; $i++) {
    ...
    $start = $i * 1000;
    $operations[] = [
      'ibm_acquia_contenthub_audit_publisher_delete',
...
$batch = [
  ...
  'finished' => 'ibm_acquia_contenthub_audit_publisher_finished',
];

$batch['file'] = '../../../../../home/ibmcom/ibm_ach_ap_batch_functions.php';
...
function ibm_ach_ap_get_entities_tracking($entity_type_id, $status_export) {
  $connection = \Drupal\Core\Database\Database::getConnection('default');
