 ...
 *   Drush Output.
 */
function ibm_acquia_contenthub_audit_publisher(array $entities, $republish, $delete, $operation_key, &$context) {
  $time = time();
...
//  $ch_entities = ibm_acquia_contenthub_audit_publisher_create_request($client_manager, 'readEntities', [$uuids]);
  $ch_entities = ibm_acquia_contenthub_audit_publisher_get_ch_entities($uuids, 10);
  foreach ($entities as $entity) {
    $out_of_sync = FALSE;
    ...
    elseif ($ch_entity && $entity->modified !== $ch_entity->getModified()) {
      ...
      if ($drupal_entity) {
        if (isEligible($entity_manager, $drupal_entity)) {
          if (!ibm_acquia_contenthub_audit_publisher_is_exported($connection, $entity)) {
          ...
          $msg = '';
          if ($delete) {
            ibm_acquia_contenthub_audit_publisher_delete_remote_entity($client_manager, $connection, $entity->entity_type, $entity->entity_uuid);
            $msg = t('. Deleting');
          }
...
//            $export_controller->exportEntities([$drupal_entity]);
  ...
  ibm_acquia_contenthub_audit_publisher_print_duration($context, $entities);
 ...
 *   The context array.
 */
function ibm_acquia_contenthub_audit_publisher_delete($start, $delete, $operation_key, &$context) {
  \Drupal::state()->set('ach_ap_running', time());
  if (empty($context['sandbox'])) {
  ...
  $list = ibm_acquia_contenthub_audit_publisher_create_request($client_manager, 'listEntities', [$options]);
  $entities = [];
 ...
 */
function ibm_acquia_contenthub_audit_publisher_finished($success, $results, $operations) {
...
function ibm_acquia_contenthub_audit_publisher_create_request($client_manager, $name, $arguments) {
  $i = 0;
  do {
    /** @var \Acquia\ContentHubClient\Entity[] $ch_entities */
...
function ibm_acquia_contenthub_audit_publisher_delete_remote_entity($client_manager, $connection, $type, $uuid) {
...
function ibm_acquia_contenthub_audit_publisher_is_exported($connection, $entity) {
...
function ibm_acquia_contenthub_audit_publisher_print_duration(&$context, $entities) {
...
function ibm_acquia_contenthub_audit_publisher_get_ch_entities($uuids, $size = 10) {
  /** @var \Drupal\acquia_contenthub\Client\ClientManager $client_manager */
  $client_manager = \Drupal::service('acquia_contenthub.client_manager');
...
function isEligible($entity_manager, $drupal_entity) {
  ...
  if ($drupal_entity instanceof NodeInterface) {
    return ibm_ach_ap_entity_is_published($drupal_entity);
  }
  return TRUE;
}
...
function ibm_ach_ap_entity_is_published($entity) {
