 ...
 *   Drush Output.
 */
function toyota_acquia_contenthub_subscriber_audit_subscriber(array $entities, $import, $operation_key, $delete, &$context) {
  $time = time();
  ...
  $ch_entities = toyota_acquia_contenthub_audit_subscriber_get_ch_entities($uuids, 10);
  ...
  foreach ($entities as $entity) {
    ...
    if (!$ch_entity) {
      ...
      if ($entity->modified !== $ch_entity->getModified()) {
        ...
        if (!toyota_acquia_contenthub_audit_subscriber_entity_exists($entity)) {
  ...
  toyota_acquia_contenthub_audit_subscriber_print_status($context['results']['total_processed'], $duration, 2, $context['results']['print']);
  // Update the last processed operation state variable.
  \Drupal::state()->set(ACH_AS_LAST_PROCESSED_OPERATION, $operation_key);
}

function toyota_acquia_contenthub_subscriber_audit_subscriber_find_missing_imports($contenthub_filter_id, $start, $size, $reimport, $operation_key, &$context) {
  \Drupal::state()->set('ach_as_running', time());
  if (empty($context['sandbox'])) {
  ...
  toyota_acquia_contenthub_audit_subscriber_print_status($context['results']['ch_total_processed'], $duration, 2, $context['results']['print']);
  \Drupal::state()->set('ach_as_last_processed_operation', $operation_key);
}
 ...
 */
function toyota_acquia_contenthub_subscriber_audit_subscriber_finished($success, $results, $operations) {
  ...
  if ($success) {
    ...
    drush_print(dt('@total entities processed from the tracking table within @duration.', [
      ...
      '@duration' => toyota_acquia_contenthub_audit_subscriber_generate_duration_message($duration),
    ]));
...
function toyota_acquia_contenthub_audit_subscriber_entity_exists($entity) {
 ...
 * @param bool $print
 */
function toyota_acquia_contenthub_audit_subscriber_print_status($total, DateInterval $duration, int $interval = 0, bool &$print = FALSE) {
...
function toyota_acquia_contenthub_audit_subscriber_get_ch_entities($uuids, $size = 25) {
  /** @var \Drupal\acquia_contenthub\Client\ClientManager $client_manager */
  $client_manager = \Drupal::service('acquia_contenthub.client_manager');
...
//function toyota_acquia_contenthub_audit_subscriber_generate_duration_message(DateTimeInterface $start_date, DateTimeInterface $end_date, int $interval = 0, bool &$print) {
//  if (empty($end_date)) {
...
function toyota_acquia_contenthub_audit_subscriber_generate_duration_message(DateInterval $duration) {
  $duration_hours = $duration->h;
  $duration_minutes = $duration->i;
  $duration_message_parts = [];
